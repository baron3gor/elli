/*
 * Custom jQuery for Megamenu Checkbox
 */
...
jQuery(document).ready(function ($) {
	if( $("body").hasClass("nav-menus-php") ){
		...
		var form = (' \
			<p class="ale-mega-menu description-wide"> \
			<label>Multi-Column Menu \
			<input class="ale-multi-column-menu widefat code" type="checkbox"></label></p> \
		');
		...
		var form_checked = (' \
			<p class="ale-mega-menu description-wide"> \
			<label>Multi-Column Menu \
			<input class="ale-multi-column-menu widefat code" checked type="checkbox"></label></p> \
		');

		var column_count = (' \
			<p class="ale-mega-menu description-wide hidden-field column-item-size"> \
			<label>Column Item Size \
			<input class="ale-multi-column-item-size widefat code" value="column-item-size-value" type="text" /></label></p> \
		');
		...
		$('.submit-add-to-menu').on('click', function() {  
			...
			$(document).ajaxStop(function() {
				...
				$(".menu-item.pending").each(function(){
					if( $(this).find(".ale-multi-column-menu").length < 1 ){
						$(this).find(".field-move").before(form);
						$(this).find(".field-move").before(column_count.replace(/column-item-size-value/i, "3")); 
					} 
				});				
		...
		$(document.body).on('click', '.ale-multi-column-menu', function() {   

			var css_field = $(this).parents(".menu-item-settings").find(".edit-menu-item-classes");
			var css_class_value = css_field.val();

			var column_count_holder = $(this).parents(".ale-mega-menu:eq(0)").next(".ale-mega-menu"); 
			var column_count = column_count_holder.find(".ale-multi-column-item-size");
		...
		$(document.body).on('keyup mouseleave', '.ale-multi-column-item-size', function() {   
