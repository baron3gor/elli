<?php
/*
 * Javascript for Blog View
 */
function ale_blog_view() {

	global $wp_query;

	$args = array(
		'nonce'       => wp_create_nonce( 'ale-blog-view-nonce' ),
		'url'         => admin_url( 'admin-ajax.php' ),
	);
	wp_enqueue_script( 'ale-blog-view',
		get_stylesheet_directory_uri() . '/js/libs/blog-view.js',
		array( 'jquery' ),
		'1.0',
		true );
	wp_localize_script( 'ale-blog-view', 'aleblogview', $args );

}

add_action( 'wp_enqueue_scripts', 'ale_blog_view' );

/*
 * AJAX Load More
 */

function ale_ajax_blog_view() {
	...
	check_ajax_referer( 'ale-blog-view-nonce', 'nonce' );
	...
	<div class="blog-wrapper-style <?php if($ale_tab_id == 0) : echo esc_attr('post-list-style', 'elli'); else : echo esc_attr('post-grid-style', 'elli'); endif; ?>">	
		<?php if ($ale_blog->have_posts()) : while ($ale_blog->have_posts()) : $ale_blog->the_post() ;?>
			<?php if(is_sticky(get_the_ID())) : $ale_post_style_clas = $ale_post_style_clas . ' ale-sticky-reload'; endif; ?>
...
add_action( 'wp_ajax_ale_ajax_blog_view', 'ale_ajax_blog_view' );
add_action( 'wp_ajax_nopriv_ale_ajax_blog_view', 'ale_ajax_blog_view' );