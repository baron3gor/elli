...
if(has_post_thumbnail() && get_the_post_thumbnail_url(get_the_ID()) != '') : ?>
    <figure class="media-list-wrapper gallery-post-wrapper">
        <?php if($ale_tab_id == 0) : ?>
            <div class="thumba-list-wrapper fade-image">
                <div <?php if ( $images ){ echo ale_wp_kses('class="gallery-post-slider owl-carousel"', 'elli'); } ?>>
                    <a href="<?php echo esc_url(get_the_permalink()); ?>">
                        <?php if($ale_blog_style == 'style3' && $ale_col_number == '2') : echo get_the_post_thumbnail(get_the_ID(), 'post-list-st3'); else : echo get_the_post_thumbnail(get_the_ID(), 'post-list'); endif;?>
                    </a>
                    <?php 
                        ...
                        if ( $images ){ $total_images = count($images); ?>
                            ...
                            <?php foreach ( $images as $attachment ) { ?>
                                <a href="<?php echo esc_url(get_the_permalink()); ?>">
                                    <?php if($ale_blog_style == 'style3' && $ale_col_number == '2') : echo wp_get_attachment_image($attachment, 'post-list-st3'); else : echo wp_get_attachment_image($attachment, 'post-list'); endif;?>
                                </a>
                            <?php } ?>
                    <?php } ?>
                </div>
                    ...
                    <div class="gallery-post-slider owl-carousel">
                        ...
                        <?php $images = get_post_meta($post->ID, 'ale_gallery_id', true);
                            ...
                            if ( $images ){ $total_images = count($images); ?>
                                ...
                                <?php foreach ( $images as $attachment ) { ?>
                                    <a href="<?php echo esc_url(get_the_permalink()); ?>">
                                        <?php if($ale_blog_style == 'style3' && $ale_col_number == '2') : echo wp_get_attachment_image($attachment, 'post-grid-col2'); else : echo wp_get_attachment_image($attachment, 'post-grid2'); endif;?>
                                    </a>
                                <?php } ?>
                        <?php } ?>
                    </div>
