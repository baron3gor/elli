jQuery(function($){
    ...
    $.fn.ale_blog_view = function(){
        var $this      = $(this),
            getMaxPage = $this.data('maxpage');

        $this.on('click', '.blog-tabs', function(){
            ...
            function ale_tabs_ajax(callback){
                if(!loading){
                    loading = true;
                    var data = {
                        action: 'ale_ajax_blog_view',
                        nonce: aleblogview.nonce,
                        tabid: getTabId,
            ...
            if(getPostWrap.css('opacity') == '1'){
                getTabs.removeClass('current-postview-tab');
                getCurrTab.addClass('current-postview-tab');
