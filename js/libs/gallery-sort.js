jQuery(function($){
    ...
    $.fn.ale_gallery_tabs = function() {
        ...
        $this.on('click', '.gallery-tab-item', function(){
            ...
            function ale_gallery_load(callback){

                if (!loading) {
                    loading = true;
                    var data = {
                        action: 'ale_ajax_gallery_sort',
                        nonce: alegallerysort.nonce,
                        post: getPostId,
                        galstyle: getStyle,
                        galpostcount: getPage,

                    };
                    $.post(alegallerysort.url, data, function (res) {
                        //console.log(res);
                        ...
                        if (res.success) {
                            ...
                            getWrapper.css('height', getHeight).html($content).imagesLoaded(function(){
                                ...
                                if(getMaxPage > 1){
                                    getGallery.after('<div class="loadmore-container"><div class="load-wrapper"><div class="loadmore_gallery fade-animation btn-style' + getStyle + '" id="true_loadmore"><span>' + aleloadmore.button_text + '</span>' + '<div class="overlay"></div>' + '</div></div></div>');
